...
local states =
{
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:PushAnimation("sleep", true)
	...
	State{
		...
		onenter = function(inst, data)
			...
			inst.AnimState:PlayAnimation("atk") --TODO FIXME(Omar): Hook in animation when it comes in
			if data ~= nil and data.target ~= nil and data.target:IsValid() then
                local pos = data.target:GetPosition()
    ...
    State{
        ...
        onenter = function(inst)
            inst.components.locomotor:Stop()
            inst:ClearBufferedAction()
            local ignoreplay = inst.AnimState:IsCurrentAnimation("walk_pst")
...
return StateGraph("umbramonkey", states, events, "idle", actionhandlers)
